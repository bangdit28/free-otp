...
function showFeedback(message, isError = false) { /* ... (Fungsi lengkap tidak berubah) ... */ }
addStockBtn.addEventListener('click', () => { /* ... (Fungsi lengkap tidak berubah) ... */ });
function updateStockCount() { /* ... (Fungsi lengkap tidak berubah) ... */ }
function listenToRanges() { /* ... (Fungsi lengkap tidak berubah) ... */ }
function deleteRange(service, country, rangeName) { /* ... (Fungsi lengkap tidak berubah) ... */ }
function deleteOldStock() { /* ... (Fungsi lengkap tidak berubah) ... */ }
...
function approveDeposit(depositId) { /* ... (Fungsi lengkap tidak berubah) ... */ }
function listenToDepositRequests() { /* ... (Fungsi lengkap tidak berubah) ... */ }
...
ordersRef.on('value', (snapshot) => {
    tableBody.innerHTML = ''; // Selalu mulai dengan tabel bersih
        ...
        for (const orderId in userOrders) {
            ...
            if (typeof order === 'object' && order !== null && (order.status === 'waiting_number' || order.status === 'waiting_otp')) {
                ...
                if (order.status === 'waiting_otp' && order.phoneNumber) {
                    // Jika status sudah menunggu OTP dan nomor sudah ADA, tampilkan nomornya.
                    phoneHTML = `<strong>${order.phoneNumber}</strong>`;
                } else {
                    // Jika status masih menunggu nomor, ATAU jika nomor belum muncul, tampilkan "Mencari...".
                    phoneHTML = `<span class="text-warning">Mencari...</span>`;
                }
                // ======================================================
