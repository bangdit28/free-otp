...
function loadCountries() {
    ...
    countriesRef.once('value', (snapshot) => {
        ...
        if (countries) {
            ...
            updateStockCount();
            listenToRanges();
        } else { countrySelect.innerHTML = '<option>Konfigurasi negara!</option>'; }
    });
}
loadCountries();
...
function showFeedback(message, isError = false) {
    ...
    setTimeout(() => { feedbackMessage.textContent = ''; feedbackMessage.className = ''; }, 4000);
}
...
addStockBtn.addEventListener('click', () => {
    ...
    numbers.forEach(number => {
        const newStockEntry = { number: number, status: 'available', rangeName: rangeName, addedAt: firebase.database.ServerValue.TIMESTAMP };
        stockPath.push(newStockEntry, (error) => {
            ...
            if (successCount === totalNumbers) {
                ...
                numberTextarea.value = ''; rangeNameInput.value = '';
                addStockBtn.disabled = false; addStockBtn.innerHTML = 'Tambah';
            }
        });
    });
});
...
function listenToRanges() {
    ...
    stockPath.on('value', (snapshot) => {
        rangeListTbody.innerHTML = '<tr><td colspan="4" class="text-center">Memuat...</td></tr>';
        const numbers = snapshot.val();
        if (!numbers) { rangeListTbody.innerHTML = '<tr><td colspan="4" class="text-center">Tidak ada range.</td></tr>'; return; }
        const aggregatedRanges = {};
        for (const stockId in numbers) {
            const numberData = numbers[stockId];
            if (!numberData.rangeName) continue;
            if (!aggregatedRanges[numberData.rangeName]) { aggregatedRanges[numberData.rangeName] = { total: 0, available: 0 }; }
            aggregatedRanges[numberData.rangeName].total++;
            if (numberData.status === 'available') { aggregatedRanges[numberData.rangeName].available++; }
        }
        rangeListTbody.innerHTML = '';
        if (Object.keys(aggregatedRanges).length === 0) { rangeListTbody.innerHTML = '<tr><td colspan="4" class="text-center">Tidak ada range.</td></tr>'; return; }
...
function deleteRange(service, country, rangeName) {
    if (!confirm(`Yakin menghapus nomor TERSEDIA dari range "${rangeName}"?`)) return;
    const stockPath = numberStockRef.child(service).child(country);
    stockPath.orderByChild('rangeName').equalTo(rangeName).once('value', (snapshot) => {
        ...
        snapshot.forEach((childSnapshot) => {
            ...
            if (stockData.status === 'available') { updates[stockId] = null; deletedCount++; }
        });
        if (deletedCount > 0) { stockPath.update(updates).then(() => { alert(`Berhasil hapus ${deletedCount} nomor.`); }); } 
        else { alert(`Tidak ada nomor tersedia untuk dihapus.`); }
    });
}
...
function deleteOldStock() {
    const service = serviceSelect.value;
    const country = countrySelect.value;
    if (!service || !country) { alert("Pilih Layanan dan Negara."); return; }
    if (!confirm(`Yakin hapus SEMUA stok lama (tanpa range) untuk ${service} - ${country}?`)) return;
    const stockPath = numberStockRef.child(service).child(country);
    stockPath.once('value', (snapshot) => {
        if (!snapshot.exists()) { alert("Tidak ada stok."); return; }
        const updates = {};
        let deletedCount = 0;
        snapshot.forEach((childSnapshot) => {
            const stockId = childSnapshot.key;
            ...
            if (!stockData.hasOwnProperty('rangeName') && stockData.status === 'available') {
                updates[stockId] = null; deletedCount++;
            }
        });
        if (deletedCount > 0) { stockPath.update(updates).then(() => { alert(`Berhasil hapus ${deletedCount} stok lama.`); }); } 
        else { alert("Tidak ada stok lama untuk dihapus."); }
    });
}
...
function assignNumberToOrder(userId, orderId, service, country) {
    ...
    stockRef.orderByChild('status').equalTo('available').limitToFirst(1).once('value', (snapshot) => {
        ...
        } else { database.ref(`/orders/${userId}/${orderId}/status`).set('out_of_stock'); }
    });
}
...
function sendOtp(button) {
    ...
    if (!otpCode) { alert('OTP tidak boleh kosong!'); return; }
    const orderRef = database.ref(`orders/${userId}/${orderId}`);
    orderRef.once('value', (snapshot) => {
        const orderData = snapshot.val();
        if (!orderData || !orderData.stockId) { console.error("Jejak stok tidak ditemukan!"); return; }
        const updates = {};
...
function attachListenersToUser(userId) {
    ...
    userOrdersRef.on('child_added', (orderSnapshot) => {
        ...
        if (order && activeStatuses.includes(order.status) && !document.getElementById(`order-${orderId}`)) {
            checkAndSetPlaceholder();
            if (order.status === 'waiting_number') { assignNumberToOrder(userId, orderId, order.serviceName.toLowerCase(), order.country); }
            const row = document.createElement('tr');
            row.id = `order-${orderId}`;
            let phoneHTML = `<span class="text-warning">Mencari...</span>`;
            if (order.status === 'waiting_otp' && order.phoneNumber) { phoneHTML = `<strong>${order.phoneNumber}</strong>`; }
    ...
    userOrdersRef.on('child_changed', (orderSnapshot) => {
        ...
        if (order.status === 'out_of_stock') { phoneCell.innerHTML = '<strong><span class="text-danger">STOK HABIS!</span></strong>'; } 
        else if (order.phoneNumber) { phoneCell.innerHTML = `<strong>${order.phoneNumber}</strong>`; }
    });
...
ordersRef.on('child_added', (userSnapshot) => {
    if (userSnapshot.val() && typeof userSnapshot.val() === 'object') {
        const userId = userSnapshot.key;
        attachListenersToUser(userId);
    }
});
ordersRef.once('value', (snapshot) => {
    if (!snapshot.exists()) { checkAndSetPlaceholder(); }
});
