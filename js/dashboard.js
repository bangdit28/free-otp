...
function loadUserOrders() {
    ...
    userOrdersRef.on('child_added', (snapshot) => {
        ...
        if (orderData.status !== 'finished_by_user') { addOrUpdateOrderRow(orderId, orderData); }
    });
    userOrdersRef.on('child_changed', (snapshot) => {
        ...
        if (orderData.status === 'finished_by_user') {
            ...
            if (row) row.remove();
        } else { addOrUpdateOrderRow(orderId, orderData); }
    });
...
function addOrUpdateOrderRow(orderId, data) {
    ...
    if (!row) { row = document.createElement('tr'); row.id = `order-${orderId}`; activeOrdersTbody.prepend(row); }
    let phoneHTML = `<div class="spinner-border spinner-border-sm"></div>`;
    if (data.status === 'out_of_stock') { phoneHTML = `<span class="text-danger fw-bold">Stok Habis</span>`; } 
    else if (data.phoneNumber) { phoneHTML = `${data.phoneNumber} <i class="bi bi-clipboard copy-icon" onclick="copyToClipboard('${data.phoneNumber}', this)"></i>`; }
    ...
    let actionHTML = `<div class="spinner-border spinner-border-sm"></div>`;
    if (data.status === 'waiting_otp' || (data.phoneNumber && !data.otpCode)) { actionHTML = `<button class="btn btn-sm btn-cancel" onclick="cancelOrder('${orderId}')"><i class="bi bi-x-circle"></i> Batal</button>`; } 
    else if (data.otpCode) { actionHTML = `<button class="btn btn-sm btn-success" onclick="finishOrder('${orderId}')"><i class="bi bi-check-lg"></i> Selesai</button>`; } 
    else if (data.status === 'out_of_stock') { actionHTML = `<i class="bi bi-x-circle-fill text-danger"></i>`; }
    row.innerHTML = `<td>${data.serviceName}</td><td class="phone-cell">${phoneHTML}</td><td class="otp-cell">${otpHTML}</td><td class="timer-cell">--:--</td><td class="status-cell">${actionHTML}</td>`;
    const createdAt = data.createdAt || Date.now();
    const timeElapsed = (Date.now() - createdAt) / 1000;
    const remainingSeconds = Math.round(600 - timeElapsed);
    if (remainingSeconds > 0) { startTimer(orderId, remainingSeconds); } 
    else {
        const timerCell = row.querySelector('.timer-cell');
        timerCell.textContent = "Expired";
        if (data.status !== 'finished_by_user') { setTimeout(() => { const row = document.getElementById(`order-${orderId}`); if (row) { row.remove(); } }, 3000); }
    }
}
getOrderBtn.addEventListener('click', () => {
    if (!currentUserId || !countrySelect.value) return;
    if (currentUserBalance < SERVICE_PRICE) { alert("Saldo tidak cukup!"); return; }
    const orderId = database.ref().child('orders').push().key;
    const newOrderData = {
        serviceName: 'Facebook', price: SERVICE_PRICE, status: 'waiting_number', phoneNumber: '',
        otpCode: '', country: countrySelect.value, createdAt: firebase.database.ServerValue.TIMESTAMP
    };
    ...
    database.ref().update(updates).catch(err => alert("Error: " + err.message));
});
function cancelOrder(orderId) {
    if (!currentUserId) return;
    if (!confirm("Yakin membatalkan? Nomor akan hangus dan saldo TIDAK dikembalikan.")) { return; }
    ...
    orderRef.once('value', (snapshot) => {
        ...
        } else { orderRef.remove(); }
    });
}
function finishOrder(orderId) { if (!currentUserId) return; database.ref(`orders/${currentUserId}/${orderId}`).update({ status: 'finished_by_user' }); }
submitDepositBtn.addEventListener('click', () => {
    const amount = parseInt(depositAmountInput.value);
    if (!amount || amount < 10000) { depositAlert.textContent = "Minimal deposit Rp 10.000."; depositAlert.className = 'alert alert-danger'; return; }
    const depositId = database.ref().child('deposit_requests').push().key;
    const currentUser = auth.currentUser;
    if (!currentUser) return;
    const depositData = { userId: currentUser.uid, userEmail: currentUser.email, amount: amount, status: 'pending', createdAt: firebase.database.ServerValue.TIMESTAMP };
    database.ref(`deposit_requests/${depositId}`).set(depositData).then(() => {
        depositAmountInput.value = '';
        depositAlert.textContent = "Permintaan berhasil diajukan.";
        depositAlert.className = 'alert alert-success';
        setTimeout(() => { depositAlert.className = 'alert d-none'; depositAlert.textContent = ''; }, 4000);
    });
});
